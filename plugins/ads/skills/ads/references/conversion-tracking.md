...
## Cross-Platform Attribution Checks

**Applicability**: Only evaluate when advertiser runs 2+ ad platforms. Mark all XP checks N/A for single-platform advertisers.

| ID | Check | Severity | Pass | Warning | Fail |
|----|-------|----------|------|---------|------|
| XP-01 | Event naming consistency | High | Same conversion events use consistent names across all platforms (e.g., "Purchase" everywhere) | Minor naming differences but events are mapped | Different event names with no mapping documentation |
| XP-02 | Conversion value consistency | High | Same conversion event reports consistent values across platforms (within 10% variance) | 10-25% variance between platforms | >25% value variance between platforms for same event |
| XP-03 | Attribution window alignment | Medium | Attribution windows intentionally set per platform with documented rationale | Default windows used without review | Conflicting windows causing double-counting (e.g., 90-day Google + 28-day TikTok overlapping) |
| XP-04 | Deduplication strategy | Critical | Documented cross-platform dedup method (CRM-based, holdout, or incrementality) | Dedup acknowledged but not implemented | No dedup strategy — sum of platform conversions used as total (inflated) |
| XP-05 | GA4 unified view | High | GA4 or equivalent analytics tool configured to show all-platform conversion paths | GA4 linked to some platforms | No unified attribution view across platforms |
| XP-06 | Budget allocation by attribution | Medium | Budget decisions informed by cross-platform attribution data (not just platform-reported ROAS) | Some cross-platform analysis done | Budget allocated purely based on each platform's self-reported metrics |

### Cross-Platform Attribution Best Practices

1. **Never sum platform-reported conversions** — overlap causes 20-40% inflation
2. **Use CRM or server-side as source of truth** for total conversions
3. **Align attribution windows intentionally** — shorter windows reduce overlap
4. **Run incrementality tests** quarterly to validate each platform's true contribution
5. **Document your dedup method** — even a simple "last-click CRM" approach is better than none
