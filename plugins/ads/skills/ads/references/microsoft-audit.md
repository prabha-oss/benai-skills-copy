...
<!-- Total Checks: 21 | Categories: 5 | See scoring-system.md for weights and algorithm -->
...
| Creative & Extensions | 20% | MS11-MS13, MS19-MS20, MS-CR1 (6 checks) |
...
| MS03 | Google Ads import validation | High | If imported: all settings verified (URLs, extensions, bids) | Minor discrepancies found | Import errors not resolved (broken URLs, missing goals) | **N/A if native Microsoft account (not imported from Google)** |
...
| MS07 | Target New Customers (PMax) | Medium | "Target New Customers" enabled for growth campaigns (Beta 2026) | — | Not tested for eligible PMax campaigns | **N/A if no PMax campaigns** |
...
| MS10 | LinkedIn profile targeting | High | LinkedIn targeting utilized for B2B (company, industry, job function) | Partial LinkedIn targeting | No LinkedIn targeting for B2B campaigns (unique advantage missed) | **N/A for B2C campaigns** |
...
| MS-CR1 | PMax asset density | High | ≥15 images, ≥3 descriptions, ≥10 headlines per asset group | 5-14 images, 2 descriptions, or 3-9 headlines | <5 images OR <2 descriptions OR <3 headlines | **N/A if no PMax campaigns** |
...
| ID | Check | Severity | Pass | Warning | Fail |
|----|-------|----------|------|---------|------|
| MS14 | Copilot placement | Medium | Copilot chat placement enabled for PMax campaigns | — | Not enabled (73% higher CTR opportunity) | **N/A if no PMax campaigns** |
...
- **PMax Asset Density**: Microsoft PMax mirrors Google's asset requirements. Fill all slots (up to 20 images, 15 headlines, 5 descriptions, 5 logos) for best Ad Strength. Video is optional — unlike Google, Microsoft does NOT auto-generate video if absent. Asset-level editorial review has been active since November 2025, so monitor for disapprovals after adding new assets. See `creative-volume.md` for full production benchmarks.
