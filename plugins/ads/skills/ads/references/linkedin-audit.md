...
<!-- Total Checks: 26 | Categories: 5 | See scoring-system.md for weights and algorithm -->
...
| Creative & Formats | 20% | L10-L13, L-CR1 (5 checks) |
...
| L07 | ABM company lists | Medium | Target company lists uploaded (up to 300,000) for ABM | Partial list uploaded | No ABM lists for enterprise campaigns | **N/A for SMB campaigns (non-ABM)** |
| L08 | Audience expansion setting | Medium | OFF for precise targeting, ON for scale (intentional) | — | Default ON without review |
| L09 | Predictive audiences | Medium | Predictive audiences tested (replaced Lookalikes Feb 2024) | — | Not tested for eligible campaigns | **N/A if audience seed <300 members** |
...
| ID | Check | Severity | Pass | Warning | Fail |
|----|-------|----------|------|---------|------|
| L10 | Thought Leader Ads (TLAs) | High | TLAs active, receiving ≥30% of budget for B2B | TLAs tested but <30% budget | No TLAs (CPC $2.29-4.14 vs $13.23 standard) | **N/A for B2C LinkedIn campaigns** |
...
| L-CR1 | Creative volume per campaign | High | 4-5 active variations per campaign; 80/20 budget split (winners/testing) | 2-3 active variations | <2 active variations OR no testing allocation |
...
| ID | Check | Severity | Pass | Warning | Fail |
|----|-------|----------|------|---------|------|
| L14 | Lead Gen Form optimization | High | ≤5 fields (reduce friction); 13% CVR benchmark | 6-8 fields | >8 fields (high friction) | **N/A if not using Lead Gen Forms** |
| L15 | Lead Gen Form CRM integration | High | Form synced to CRM in real-time | Synced within 24hrs | Manual CSV download only | **N/A if not using Lead Gen Forms** |
...
| L20 | Frequency monitoring | High | Message frequency ≤1 per 30-45 days per user | 1 per 20-30 days | >1 per 20 days (inbox fatigue) | **N/A if not running Message Ads** |
...
- **Creative Volume & Refresh Cadence**: LinkedIn campaigns perform best with 4-5 active ad variations per campaign, allocating 80% of budget to proven winners and 20% to testing new creative. Maximum 15 active ads / 100 total per campaign. Refresh every 2-4 weeks, with the "magic number" for conversion at 5-9 touches per prospect. See `creative-volume.md` for full production benchmarks.
