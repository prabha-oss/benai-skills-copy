...
| G45 | Consent Mode v2 (EU/EEA) | Critical | Consent Mode v2 implemented with Advanced mode | Basic mode only | Not implemented (if serving EU/EEA) | **N/A if no EU/EEA targeting** |
...
| G06 | PMax present for eligible accounts | Medium | PMax active for accounts with conversion history | PMax tested but paused | No PMax tested despite eligibility | **N/A if no PMax campaigns** |
| G07 | Search + PMax overlap | High | Brand exclusions configured in PMax when Search brand campaign exists | Partial brand exclusions | No brand exclusions in PMax alongside brand Search | **N/A if no PMax campaigns** |
...
| G10 | Ad schedule configured | Low | Ad schedule set if business has operating hours | — | No schedule despite clear business hours | **N/A if 24/7 or national business** |
| G11 | Geographic targeting accuracy | High | "People in" (not "People in or interested in") for local | — | "People in or interested in" for local business | **N/A if national/non-local business** |
...
| G31 | PMax asset group density | Critical | ≥15 images, ≥3 logos, ≥3 videos per group ("Excellent" Ad Strength) | 5-14 images, 1-2 logos, or 1-2 videos | <5 images OR 0 logos OR 0 video | **N/A if no PMax campaigns** |
| G32 | PMax video assets present | High | Native video in all formats (16:9, 1:1, 9:16) | 1-2 formats only | No native video (auto-generated only) | **N/A if no PMax campaigns** |
| G33 | PMax asset group count | Medium | ≥2 asset groups per PMax (intent-segmented) | 1 asset group | — | **N/A if no PMax campaigns** |
| G34 | PMax final URL expansion | High | Configured intentionally (ON for discovery, OFF for control) | — | Default ON without review | **N/A if no PMax campaigns** |
...
| G54 | Call extensions (if applicable) | Medium | Call extensions with call tracking for phone-based businesses | Call extension without tracking | No call extension for phone-based business | **N/A if non-phone business** |
| G55 | Lead form extensions | Low | Lead form tested for lead gen accounts | — | Not tested | **N/A if non-lead-gen business** |
...
| ID | Check | Severity | Pass | Warning | Fail |
|----|-------|----------|------|---------|------|
| G-PM1 | Audience signals configured | High | Custom audience signals per asset group | Generic signals only | No audience signals | **N/A if no PMax campaigns** |
| G-PM2 | PMax Ad Strength | High | "Good" or "Excellent" | "Average" | "Poor" | **N/A if no PMax campaigns** |
| G-PM3 | Brand cannibalization | High | <15% of PMax conversions from brand terms | 15-30% from brand terms | >30% from brand terms | **N/A if no PMax campaigns** |
| G-PM4 | Search themes | Medium | Search themes configured (up to 50 per asset group) | <5 search themes | No search themes | **N/A if no PMax campaigns** |
| G-PM5 | Negative keywords | High | Brand + irrelevant negatives applied (up to 10,000) | Some negatives applied | No negative keywords in PMax | **N/A if no PMax campaigns** |

---

## Context Notes

- **PMax Ad Strength & Asset Density**: Filling all asset slots (up to 20 images, 5 logos, 5 videos, 15 headlines, 5 descriptions) achieves "Excellent" Ad Strength, which delivers ~12-15% more conversions than "Poor". Google rates uniqueness of headlines (Levenshtein distance), keyword inclusion, and format coverage. 3-7 headlines = Poor/Average, 8-10 = Good, 11-15 = Excellent. Video expansion from 5 to 15 per asset group is in testing for 2026. See `creative-volume.md` for full production benchmarks.
