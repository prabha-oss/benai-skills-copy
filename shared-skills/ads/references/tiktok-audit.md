...
|----|-------|----------|------|---------|------|
| T05 | Creative volume | Critical | ≥6 creative assets per ad group (top performers: 10-20/campaign; Smart+ launch: ≥4 high-quality) | 3-5 creatives | <3 creatives per ad group |
...
| T09 | Creative lifespan | High | No creative active >7 days with declining CTR; no creative active >30 days regardless | 7-14 days with minor decline OR creative active 21-30 days | >14 days with significant CTR decline OR any creative active >30 days regardless of CTR |
...
| T20 | TikTok Shop integration | Medium | Shop catalog connected (for e-commerce) | — | Eligible but not connected | **N/A if not e-commerce or not in eligible TikTok Shop country** |
| T21 | Video Shopping Ads (VSA) | Medium | VSA tested for product catalog accounts | — | Not tested despite eligible catalog | **N/A if not e-commerce or no product catalog** |
...
**Validation tool**: Run `python3 ~/.claude/skills/ads/scripts/analyze_landing.py <landing-page-url>` to automatically test ttclid capture. The script navigates with a test ttclid parameter and checks localStorage, sessionStorage, cookies, and dataLayer for capture confirmation.
...
| T04 | Smart+ utilization | Medium | Smart+ campaigns tested with modular automation (per-module: targeting, budget, creative, placement) | — | Not tested (42% adoption, 1.41-1.67 median ROAS; modular since Oct 2025) | **N/A if intentionally running manual campaigns (document reason)** |
...
| T16 | Dayparting | Low | Ad schedule aligned with target audience activity | — | No schedule despite clear patterns | **N/A if always-on strategy with no clear activity patterns** |
...
- **Creative Fatigue Timeline**: TikTok creative fatigue begins at 3-5 days, with typical effective lifespan of 7-10 days. Refresh every 10-14 days by adding new creatives to EXISTING ad groups (preserves learnings). Never create new ad groups for refreshes. High-scale accounts need new creatives twice per week. See `creative-volume.md` for full production benchmarks.
